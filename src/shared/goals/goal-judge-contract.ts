...
export function judgeRunsWholeGoal(spec: Pick<GoalSpec, 'judge' | 'criteria'>): boolean {
  return (spec.judge ?? 'none') !== 'none' && spec.criteria.every((c) => Boolean(c.command))
}
...
export function composeGoalAcceptanceText(
  spec: Pick<GoalSpec, 'objective' | 'criteria' | 'acceptanceText'>
): string {
