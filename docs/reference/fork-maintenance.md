...
7. **每个功能都必须同时覆盖本地访问和 SSH 访问两种场景,缺一个就不算完成。** SSH 场景指:Orca 客户端在一台机器上,工作区、文件或服务在通过 SSH 连接的另一台机器上(例:在 mbp5 的 Orca 里通过 SSH 打开 minizc 上的文件)。
   - 需求:`requirements/*.md` 的每条 REQ 都要分别写出本地场景和 SSH 场景的行为与验收;只写一种就是没写完。
   - 方案:逐项写清“这件事在哪台机器上执行”——读写的文件、访问的 `127.0.0.1` 与端口、环境变量、配置、调用的服务,分别属于客户端还是 SSH 主机;SSH 断开或未连接时怎么表现。不能默认“本机访问本机”。
   - 测试:`tests/cases/*.md` 必须有 SSH 场景用例;真机验证要在 SSH 场景下跑一遍,不能只在本机验。
   - 反例:自托管产物后端(2026-09-10)把服务地址写死成 `127.0.0.1:8787`,只在装服务的那台机器上验过。SSH 场景下客户端把请求发给了自己本机的 8787,被那里的 llm-proxy 转发到内部上游服务,文档正文和 Orca 账号 token 一起发了出去,分享失败。
