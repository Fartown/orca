...
2. **沙箱是可选的,默认不加**。判据常常需要构建、起服务、跑测试才验得了,锁成只读会把裁判废掉,
   所以 `--sandbox` 要显式给(`read-only` / `workspace-write` / `danger-full-access`)。
   **不给的时候,「裁判不改文件」只是提示词里的一句请求,不是机制** —— 而 codex 的全局配置
   可能本身就是 `danger-full-access`。要机制保证就显式传 `--sandbox read-only`。
...
- **显式传 `--sandbox read-only` 时,codex 裁判确实改不了文件**:故意给它「改文件就算达成」的标准,
  它报 FAIL 且文件哈希未变。**不传时没有这层保证** —— 那次验证是在显式加了沙箱的前提下做的
