...
- 不修 AI Vault 右侧已有的重复 Resume 或 Host 问题；原 pane 识别的 launch-config 身份缺口
  已按 2026-08-31 决策收进本需求（见变更记录与独立方案）；
...
| `OriginalPaneState` 扩读 launch-config     | 做（2026-08-31 决策） | 见 `.docs/并行任务看板/方案-原pane识别-补launch-config.md`；扩读 pending startup 仍不做 |
...
### 2026-09-01 · 标题生命周期落库与全局共享（复核收边）

- 变更内容（另一会话实现，本次复核收边）：`conversations.title_source`（v3 迁移，minted/provider/user
  三态）+ 单一标题写入门（用户命名冻结自动来源、自动来源可互相跟随、mint 至多一次）；tab 标题与右侧
  历史行展示 canonical 会话名。此项有意偏离本方案"不新增列/migration"预算——schema 尚未发布，v3 为
  首个真实迁移。
- 本次收边：右侧曾直接 import `@/issues/conversation-canonical-titles`（破坏依赖边界），改为中性
  `lib/canonical-session-titles` 注册表——Issues 在 `IssueDomainSyncGate` 挂载时注册 provider，
  AI Vault 只读中性模块；`AiVaultPanel` 抽出 `use-canonical-session-titles` hook（顺带解决
  max-lines 超限）；边界 ratchet 测试与门禁清单同步收编；`pane-agent-identity-inventory` 计数 +1。
- 记录人：Claude。

### 2026-08-31 · 收进原 pane 识别的 launch-config 修复（Claude）

- 变更原因：真实事故证明 resume 后空闲的 agent 在首轮 hook 前对查找器不可见，导致二次 Resume
  与 codex writer 冲突（-32600）。
- 变更内容：pane 绑定/冷恢复注册 launch-config 时写入 `resumeProviderSession`（填既有可选字段）；
  `findOriginalAiVaultSessionPane` 与索引在 sleeping 之后增加 launch-config 第四来源；
  actions 订阅补一个 map。远端 presentation 载荷不含身份，远端 v1 不覆盖、不改 wire。
  详细代码方案：`.docs/并行任务看板/方案-原pane识别-补launch-config.md`。
- 记录人：Claude。
