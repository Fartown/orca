...
当前实现只扩展既有产物 API 的 origin 白名单：私有 IPv4 和 `.local` 可使用 HTTP/HTTPS，公网仍限第一方 HTTPS。URL 解析、鉴权、Profile 隔离、发布器和网络请求继续走原服务。核对基线与目标见 [主需求](../requirements/自托管产物后端.md)；本文件是当前实现说明，不是待实施的新后端方案。

关联需求：REQ-201、REQ-202、REQ-203、REQ-204。当前已确认配置与请求链；真实网络、后端部署与远端进程环境未验证。
...
| 模块 | 变更归属 | 输入与输出 | 复用结论 |
| --- | --- | --- | --- |
| `artifact-cloud-config.ts` | 本需求修改 | override/env → origin | 在原 resolver 上增加私有主机判断；不创建第二个配置系统 |
| `artifact-cloud-config.test.ts` | 本需求修改 | origin 样例 → 精确结果或错误 | 扩展原测试，覆盖私网边界和伪造域名 |
| `ArtifactCloudService.withAuth` | 既有能力 | origin + active profile → 操作上下文 | 沿用登录、刷新、profile fence 和显式 override 限制 |
...
├── artifact-cloud-service.ts      [复用] profile 与登录
...
| 项目 | 当前行为 |
| --- | --- |
| 优先级 | `override?.trim() || env.ORCA_ARTIFACTS_API_URL?.trim() || production default` |
