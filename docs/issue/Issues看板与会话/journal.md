...
| 公共依赖 | [会话命名与身份保护](../会话命名与身份保护/journal.md) | ready | 独立需求入口；Issues 只消费名称与身份，不拥有该功能 |
...
- 日期：2026-09-10。
- 背景：用户明确指出公共会话命名不归 Issues，要求独立拆出。
- 备选项：继续混在 Issues；或独立公共能力，Issues 引用。
- 最终决定：REQ-025/026/028、D-003～D-006、命名方案/调研/测试和开发记录迁到[独立 Journal](../会话命名与身份保护/journal.md)，编号与证据保留。
- 原因：命名是公共能力，Issues 只是筛选、组织和绑定入口。
- 影响范围：文档与功能归属；不删除运行时功能、不改业务规则或历史证据。
...
- 本轮目标：执行用户要求，独立管理会话命名与身份保护。
- 完成内容：迁移已有命名文档、决策和开发记录；Issues 需求仅保留消费关系及独立需求链接。
- 代码或文档变更：功能注册与架构范围同步拆分；没有改动产品代码。
- 验证证据：本轮目录、链接与登记校验见独立命名 Journal；旧测试证据保持原样。
- 未解决问题：此前 PR 尚未合入，不将文档迁移称为产品重新验收。
- 下一步：以独立需求为命名工作的事实入口；本需求继续只维护 Issues 业务。
