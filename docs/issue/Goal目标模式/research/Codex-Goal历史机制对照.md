...
> 历史研究，冻结于下述 Codex SHA。本轮保留已去重的完整机制研究，未重新核对外部 Codex 仓库；本文的功能、文件数、测试数和工具语义不代表当前 Codex 或当前 Orca。Orca 当前实现以 [Goal 技术说明](../solutions/Goal目标模式技术说明.md) 为准。

调研对象:`openai/codex` @ `2b5bdcf6`(2026-08-02)
覆盖:goal 相关全部 53 个文件、6 个实现层、20 个后端集成测试、3 份提示词、5 次数据库迁移

本文合并了此前的 4 份调研笔记(首版调研、精读修正、防敷衍机制分析、完整解读)，仅作为上述历史版本的完整参考。
我们自己的当前实现见 [Goal 技术说明](../solutions/Goal目标模式技术说明.md)。

> 文档约定:占位符用大写下划线,不用尖括号。
...
我们改写后的运行模板见 [`goal-mode/cli/prompts/`](../../../../goal-mode/cli/prompts)。
...
(当前实现与实际限制见 [Goal 技术说明](../solutions/Goal目标模式技术说明.md))。
