...
- 关联需求：[REQ-001～REQ-030](../../requirements/Issues看板与会话.md)。技术合同见[技术说明](../../solutions/Issues看板与会话技术说明.md)。
...
当前共 237 条规格，P0=78，P1=121，P2=38。原 TC-001～TC-190 保留原优先级分布 59/101/30；新增 TC-191～TC-230 覆盖行复用、恢复、标题、迁移与后续目标；TC-231～TC-237 覆盖变化通知与不可达主机（REQ-030）。
...
| 全量P0   | 78 条                                                                                                                                                        | 主链与隔离/兼容门禁                                  |
| P0+P1    | 199 条                                                                                                                                                       | 核心回归、schema/host/标题改动                       |
| 全量规格 | 237 条                                                                                                                                                       | 大改动覆盖；后续目标项单列，不用自动跳过冒充全量完成 |
...
| F-11    | REQ-018、030                | 变化通知、读取时机与不可达主机               | P0: 2 / P1: 4 / P2: 1  | 按变化读取、离线零请求、重连补齐                  | TC-231～TC-237 |
...
| TC-225 route独立刷新目标          | REQ-017、REQ-029          | P2 / 回归     | 一条route慢于轮询周期，另一条稳定                                   | 并发观察多轮刷新            | 每个route同时只有一次读取，读取期间的通知合并为一次补读；慢route不阻塞其他route，也不让旧响应覆盖新结果（REQ-030） |
...
| TC                                | 关联需求         | 优先级 / 类型 | 前置条件与测试数据                               | 操作步骤                                   | 预期结果                                                                                             |
| --------------------------------- | ---------------- | ------------- | ------------------------------------------------ | ------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| TC-231 不可达主机零请求           | REQ-018、REQ-030 | P0 / 网络     | H-RUNTIME-WT；主机进程被杀，或地址无法到达       | 保持客户端运行，主机状态反复重新发布       | 该 route 显示 offline；期间没有任何 `issues.*`/`conversations.*` 请求；CPU 与内存平稳，不随时间增长  |
| TC-232 主机写入后客户端按通知读取 | REQ-030          | P0 / 同步     | H-RUNTIME-WT 在线且安静                          | 观察一段时间，再在主机上创建 Issue         | 安静期没有 Issue 请求；创建后 1 秒内客户端读取该 route，看到新 Issue                                 |
| TC-233 断线重连后补齐             | REQ-030          | P1 / 网络     | 在线 paired route，已有快照                      | 断开期间在主机写入，再恢复连接             | 恢复后订阅 ready 触发一次读取，断线期间的写入出现；断线期间不发读取请求                             |
| TC-234 通知合并与单 route 串行    | REQ-017、REQ-030 | P1 / 并发     | 一次读取进行中                                   | 连续到达多条该 route 的通知                | 同一 route 不并发读取；读取结束后只补读一次                                                         |
| TC-235 新客户端连旧主机按需读取   | REQ-018、REQ-030 | P1 / 混合版本 | 主机支持 Issues 但没有 `issues.subscribeChanges` | 打开 route，断开，再重连                   | 退回为 5/15 秒读取，仅在联系期间；失去联系后停止读取；重连后重新尝试订阅                            |
| TC-236 SSH 分区由本机通知         | REQ-015、REQ-030 | P1 / SSH      | H-SSH-WT 已连接                                  | 在该 SSH 分区创建 Issue；再断开 SSH        | 创建后本机通知只触发 SSH route 的读取；SSH 断开和重连不引发额外轮询                                  |
| TC-237 远端订阅关闭即清理         | REQ-030          | P2 / 资源     | 在线 paired route                                | 移除 route 或渲染进程重载，连接保持        | 客户端发送 `issues.unsubscribeChanges`；主机侧该订阅结束，不留监听到连接关闭                        |
...
- 原 REQ-001～REQ-029 的历史映射保留，REQ-030 由 TC-231～TC-237 覆盖；REQ-025/026/028 的当前验收由独立命名需求拥有。
