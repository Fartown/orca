...
describe('registerOrcaProfileHandlers', () => {
  ...
  it('does not query Issue facts for Project copy', async () => {
    const guard = {
      findManagedConversationsForRepo: vi.fn(() => {
        throw new Error('unused')
      })
    }
    const result = { status: 'transferred', mode: 'copy' }
    getOrcaProfileListStateMock.mockReturnValue({
      activeProfileId: 'personal',
      profiles: []
    })
    ...
    expect(guard.findManagedConversationsForRepo).not.toHaveBeenCalled()
  })

  it('blocks Project move before flush or transfer when managed Conversations exist', async () => {
    const flush = vi.fn()
    const guard = {
      findManagedConversationsForRepo: vi.fn(() => [
        {
          conversationId: 'conversation-1',
          workspaceRef: { type: 'worktree' as const, worktreeId: 'repo-1::/tree' },
          title: 'Managed work'
        }
      ])
    }
    ...
    ).rejects.toThrow('issue_project_move_blocked:1:Managed work')
    expect(guard.findManagedConversationsForRepo).toHaveBeenCalledWith({
      profileId: 'personal',
      repoId: 'repo-1'
    })
