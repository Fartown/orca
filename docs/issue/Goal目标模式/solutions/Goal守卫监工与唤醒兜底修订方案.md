...
- 客户端新增应用级的 `GoalNoticeWatcher`：不依赖目标面板是否打开，对本机和每个出现在已知工作区里、且当前在联系的执行主机，每 30 秒拉一次目标（2026-09-25 起不在线的主机不拉，恢复联系时立即拉一次）；遇到没显示过的事件，就调用现有的 `window.api.notifications.dispatch` 发系统通知。复用的是「agent 任务完成」这一类通知（问题、故障类显示为需要处理，完成类显示为完成），因此遵守用户对这一类的开关，也会同步到已配对的手机；只有被突发冷却吞掉的会重试。已显示的事件编号记在客户端本地。
