...
describe('orca CLI skill guidance', () => {
  ...
  // Sharing defaults to off, so an agent that follows the unconditional share workflow
  // just loops on denials. The guide has to teach the opt-in and the recovery.
  it('teaches the artifact share opt-in and its recovery path', () => {
    // Normalized so the assertions survive reflowing the guide's prose.
    const skill = readSkill().replace(/\s+/gu, ' ')

    expect(skill).toContain('**Sharing is off by default and only a human can turn it on.**')
    expect(skill).toContain('Settings → Artifacts')
    expect(skill).toContain('Share on local network from this computer')
    ...
    // The gate is per computer, and revocation surfaces stay reachable.
    expect(skill).toContain('every caller on that computer, agent or human')
