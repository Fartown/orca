...
/** Bumped to drop results from a refresh that a newer one (or teardown) superseded. */
export type IssueRefreshSequenceRef = { current: number }
...
export async function refreshRoute(
  ...
  loadIssues: boolean,
  sequence: number,
  sequenceRef: IssueRefreshSequenceRef
): Promise<void> {
