...
## 2026-09-12 — Linux 红绿对照通过，补齐长期覆盖

- 本轮目标：保留有效修复，并确保定时全量 CI 不因 `@headful` 分组变化而遗漏此契约。
- 完成内容：SHA `615257980ea1aac7d7b24f78259a0621b1734011` 的 run `34678584582` / job `103512918693` 原 5 轮全部通过；窗口由 CI 隔离 Xvfb 非激活显示，5 次 `visible=true`、`backgroundThrottling=true`。恢复耗时 220.3/160.0/154.6/249.2/151.2 ms，中位数 160 ms；首次挂载均为 1，其余 tab 均恢复。
- 代码或文档变更：全量 CI 在第 1/14 分片额外执行此单一 headful spec，原 headless 分片不变；PR 仍复用既有变更用例入口。完成采样后附加 PNG，CI 保存成功附件；新增两项工作流契约测试和精确基础设施路径登记。
