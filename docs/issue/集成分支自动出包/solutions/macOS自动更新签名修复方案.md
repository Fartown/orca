...
- 按用户 2026-09-13 最新要求，本轮原生验收只覆盖 Apple Silicon（arm64），不以 Intel 为阻塞项。若仍不被接受，记录原始原因；不以文档推断替代实测。
...
- 同一身份构建真实 arm64 Orca A/B，使用临时安装目录和独立 profile，原生隐藏窗口验证；不得操作用户正在使用的 App。
